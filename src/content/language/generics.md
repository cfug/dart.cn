...
你会看到该类型实际上是 `List<E>`。\<...\> 符号将 List 标记为
**泛型**（或**参数化**）类型——一种具有形式类型参数的类型。
[按照惯例][By convention]，大多数类型变量都有单字母名称，
如 E、T、S、K 和 V。
...
如果你想让列表只包含字符串，你可以将其声明为 `List<String>`（读作"字符串列表"）。
这样，你、你的同事程序员和你的工具就可以检测到将非字符串赋值给列表可能是一个错误。
以下是一个示例：
...
你发现你想要这个接口的字符串特定版本，所以你创建了另一个接口：
...
在这段代码中，T 是替代类型。它是一个占位符，你可以把它看作是开发人员稍后会定义的类型。
...
List、set 和 map 字面量可以被参数化。参数化字面量与你已经见过的字面量一样，
只是在左括号之前添加 <code>&lt;<em>type</em>></code>（用于 list 和 set）
...
在使用构造函数时指定一个或多个类型，将类型放在类名后面的尖括号（`<...>`）中。例如：
...
以下代码创建一个 `SplayTreeMap`，它具有整数键和 `View` 类型的值：
...
Dart 泛型类型是*具体化的*，这意味着它们在运行时携带其类型信息。
例如，你可以测试集合的类型：
...
相比之下，Java 中的泛型使用*擦除*，这意味着泛型类型参数在运行时被移除。
在 Java 中，你可以测试对象是否是 List，但不能测试它是否是 `List<String>`。
:::
...
（而不是默认的 [`Object?`][top-and-bottom]）来确保类型非空。
...
你可以使用 `extends` 与 `Object` 以外的其他类型。
以下是扩展 `SomeBaseClass` 的示例，
以便可以在类型 `T` 的对象上调用 `SomeBaseClass` 的成员：
...
当使用约束来限制参数类型时，你可以将约束引用回类型参数本身。
这会创建一个自引用约束，或称 F-界限。例如：
...
F-界限 `T extends Comparable<T>` 意味着 `T` 必须能够与自身进行比较。
...
这里 `first` 上的泛型类型参数（`<T>`）允许你在多个地方使用类型参数 `T`：
