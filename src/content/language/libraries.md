...
`import` 和 `library` 指令可以帮助你创建模块化和可共享的代码库。
库不仅提供 API，还是一个隐私单元：以下划线（`_`）开头的标识符仅在库内可见。
*每个 Dart 文件（及其部分）都是一个[库][library]*，
即使它不使用 [`library`](#library-directive) 指令。

Libraries can be distributed using [packages](/tools/pub/packages).

库可以使用[包](/tools/pub/packages)分发。
...
Dart 使用下划线代替 `public`、`protected` 或 `private` 等访问修饰符关键字。
虽然其他语言的访问修饰符关键字提供更细粒度的控制，
但 Dart 使用下划线和基于库的隐私机制提供了直接的配置方式，
有助于实现[动态访问][dynamic access]的高效实现，并改善 tree shaking（死代码消除）。
...
使用 `import` 来指定如何在另一个库的作用域中使用来自一个库的命名空间。

For example, Dart web apps generally use the [`dart:js_interop`][]
library, which they can import like this:

例如，Dart web 应用通常使用 [`dart:js_interop`][] 库，可以这样导入它：
...
`import` 唯一需要的参数是指定库的 URI。
对于内置库，URI 使用特殊的 `dart:` 方案。
...
如果你正在构建 Flutter 应用，请参阅 Flutter 指南中关于
[延迟组件][flutter-deferred]的延迟加载实现。
...
在前面的代码中，`await` 关键字会暂停执行，直到库加载完成。
有关 `async` 和 `await` 的更多信息，请查看[异步编程](/language/async)。

You can invoke `loadLibrary()` multiple times on a library without problems.
The library is loaded only once.

你可以在同一个库上多次调用 `loadLibrary()` 而不会出问题。该库只加载一次。
...
* 延迟库的常量在导入文件中不是常量。
  记住，这些常量在延迟库加载之前不存在。
* 你不能在导入文件中使用延迟库的类型。
  相反，考虑将接口类型移到延迟库和导入文件都导入的库中。
* Dart 会将 `loadLibrary()` 隐式插入到你使用
...
有关如何实现包的建议，请参阅[创建包](/tools/pub/create-packages)，包括：
