...
function updatePlaceholders(channel, version) {
  ...
  const download = 'https://storage.flutter-io.cn/dart-archive/channels/' + channel + '/release/latest/linux_packages/dart_' + version + '-1_amd64.deb';
  if (channel === 'stable') {
...
function fetchVersion(channel) {
  fetch('https://storage.flutter-io.cn/dart-archive/channels/' + channel + '/release/latest/VERSION')
